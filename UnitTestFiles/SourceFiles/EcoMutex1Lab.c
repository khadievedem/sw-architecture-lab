...
int16_t EcoMain(IEcoUnknown* pIUnk) {
    ...
    result = pIBus->pVTbl->QueryComponent(pIBus, &CID_EcoMutex1Lab, 0, &IID_IEcoMutex1, (void**) &pIEcoMutex1);
    if (result != 0 || pIEcoMutex1 == 0) {
        /* Освобождение интерфейсов в случае ошибки */
        goto Release;
    }


    pIEcoMutex1->pVTbl->Lock(pIEcoMutex1, &output_mutex);
    printf("hello world!\n");
    pIEcoMutex1->pVTbl->UnLock(pIEcoMutex1, &output_mutex);
...
Release:
    ...
    if (pIEcoMutex1 != 0) {
        pIEcoMutex1->pVTbl->Release(pIEcoMutex1);
    }
